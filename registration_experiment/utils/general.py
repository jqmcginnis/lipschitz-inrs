...
def save_nifti_with_sitk(numpy_array, voxel_spacing, output_path, origin=None):
    """
    Save numpy array as NIfTI file using SimpleITK
    
    Parameters:
    - numpy_array: 3D numpy array (x, y, z)
    - voxel_spacing: list/tuple of [x, y, z] voxel dimensions in mm
    - output_path: string path for output file
    - origin: optional origin point [x, y, z] in mm
    """
    ...
    # Set origin (default to [0, 0, 0] if not specified)
    if origin is None:
        origin = [0.0, 0.0, 0.0]
    sitk_image.SetOrigin(origin)
    
    # Set direction (identity matrix for standard orientation)
...
def compute_landmarks(network, landmarks_pre, image_size):
    # Always work with tensors
    scale_of_axes = torch.FloatTensor([(0.5 * s) for s in image_size])

    # Ensure landmarks_pre is a tensor
    if isinstance(landmarks_pre, torch.Tensor):
        coordinate_tensor = landmarks_pre.float()  # Ensure float type
    else:
        coordinate_tensor = torch.FloatTensor(landmarks_pre)

    # Move both tensors to the same device (GPU)
    coordinate_tensor = coordinate_tensor.cuda()
    scale_of_axes = scale_of_axes.cuda()

    # Apply the transformation
    coordinate_tensor = (coordinate_tensor / scale_of_axes) - 1.0
    output = network(coordinate_tensor)  # Already on GPU, no need for .cuda()

    delta = output.cpu().detach().numpy() * scale_of_axes.cpu().numpy()  # Move back to CPU for numpy

    # Convert landmarks_pre to numpy if it's a tensor
    if isinstance(landmarks_pre, torch.Tensor):
        landmarks_numpy = landmarks_pre.cpu().detach().numpy()
    else:
        landmarks_numpy = landmarks_pre

    return landmarks_numpy + delta, delta

def load_image_DIRLab(variation=1, folder="", out_dir=""):
    ...
    image_mask = torch.from_numpy(mask)

    # save images as fixed and moving
    save_nifti_with_sitk(image_insp, voxel_sizes[variation], os.path.join(out_dir, f"case{variation}_fixed.nii.gz"))
    save_nifti_with_sitk(image_exp, voxel_sizes[variation], os.path.join(out_dir, f"case{variation}_moving.nii.gz"))
    save_nifti_with_sitk(image_mask, voxel_sizes[variation], os.path.join(out_dir, f"case{variation}_mask.nii.gz"))
