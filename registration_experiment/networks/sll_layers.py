...
class NormalizedAffineLayer(nn.Module):
    ...
    def forward(self, x):
        # Compute L2 norm of weight vector
        w_norm = torch.norm(self.weight, p=2)
        # Normalize weight and apply affine transformation
        normalized_weight = self.weight / w_norm
        return torch.matmul(x, normalized_weight) + self.bias
...
class KLipschitzSDPBasedLipschitzLinearLayer(nn.Module):
    def __init__(self, cin, inner_dim, k_coef_lip=1.0):
        super(KLipschitzSDPBasedLipschitzLinearLayer, self).__init__()
        ...
        self.k_coef_lip = torch.tensor(float(k_coef_lip))
    ...
    def forward(self, x):
        ...
        # Scale the output by the k_coef_lip
        return self.k_coef_lip * out
