...
class SpectralNorm:
    ...
    def compute_weight(self, module: Module, do_power_iteration: bool) -> torch.Tensor:
        ...
        # weight = weight / sigma
        # for k-Lipschitz
        weight = weight * (self.lipschitz_const / sigma)
        return weight
    ...
    @staticmethod
    def apply(
        module: Module, name: str, n_power_iterations: int, dim: int, eps: float, lipschitz_const: float = 1.0
    ) -> "SpectralNorm":
        ...
        fn = SpectralNorm(name, n_power_iterations, dim, eps, lipschitz_const)
    ...
    def __call__(
        ...
        if version is None or version < 1:
            ...
            with torch.no_grad():
                ...
                # Check if this is an old checkpoint (k=1) or new k-Lipschitz
                if hasattr(fn, 'lipschitz_const') and fn.lipschitz_const != 1.0:
                    # New k-Lipschitz: weight = weight_orig * (k / sigma)
                    sigma = (weight_orig * fn.lipschitz_const / weight).mean()
                else:
                    # Old standard spectral norm: weight = weight_orig / sigma
                    sigma = (weight_orig / weight).mean()
...
def spectral_norm(
    ...
    Args:
        ...
        eps (float, optional): epsilon for numerical stability in
            calculating norms
        lipschitz_const (float, optional): desired Lipschitz constant, default is 1.0
    ...
    if dim is None:
        if isinstance(
            ...
            dim = 0
    SpectralNorm.apply(module, name, n_power_iterations, dim, eps, lipschitz_const)
    return module
