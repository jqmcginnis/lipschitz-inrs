...
def compute_bending_energy(input_coords, output, batch_size=None):
    ...
    dx_xyz = torch.zeros(input_coords.shape[0], 3, 3).cuda()
    dy_xyz = torch.zeros(input_coords.shape[0], 3, 3).cuda()
    dz_xyz = torch.zeros(input_coords.shape[0], 3, 3).cuda()
    for i in range(3):
...
def compute_jacobian_matrix(input_coords, output, add_identity=True):
    ...
    for i in range(3):
        ...
        if add_identity:
            jacobian_matrix[:, i, i] += torch.ones_like(jacobian_matrix[:, i, i]).cuda()
