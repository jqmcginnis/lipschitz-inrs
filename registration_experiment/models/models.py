...
from networks.models_constrained import SirenMLP, ReluFFN
from networks.models_unconstrained import ReluFFN as Unconstrained_ReluFFN
...
class ImplicitRegistrator:
    ...
    def __init__(self, moving_image, fixed_image, **kwargs):
        ...
        if self.network_from_file is None:
            ...
            elif self.network_type == "Lipschitz_Siren":
                print(self.layers)
                self.network = SirenMLP(input_dim=3, hidden_dim=kwargs["hidden_dim"], 
                                         num_layers=kwargs["num_layers"], k_layers=kwargs["k_layers"], 
                                         k_act=kwargs["k_act"], norm_type=kwargs["norm_type"])
                
            elif self.network_type == "Lipschitz_FFN":
                print(self.layers)
                self.network = ReluFFN(input_dim=3, hidden_dim=kwargs["hidden_dim"], 
                                         num_layers=kwargs["num_layers"], k_ff=kwargs["k_fourier"],
                                         k_layers=kwargs["k_layers"], 
                                         norm_type=kwargs["norm_type"],
                                         mapping_size=128)
                
            elif self.network_type == "Vanilla_FFN":
                self.network = Unconstrained_ReluFFN(input_dim=3, hidden_dim=kwargs["hidden_dim"], 
                                         num_layers=kwargs["num_layers"], sigma= kwargs["k_fourier"],
                                         mapping_size=128)
            else:
            ...
            if self.gpu:
                self.network.cuda()
        print("running on:", "GPU" if self.gpu else "CPU")
